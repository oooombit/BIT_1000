    ...
    */
    function getHandleCmd() {
        ...
        if (handleCmd.charAt(0).compare("F") == 0) {
            if (handleCmd.charAt(1).compare("E") == 0)
                if (handleCmd.charAt(2).compare("F") == 0)
                    if (handleCmd.charAt(3).compare("E") == 0)
                        flag = 1;
        }
        // let first_index = findIndexof(handleCmd, "F", 0);
        // serial.writeString(first_index.toString());
        if (flag != -1) {
            let index = strToNumber(handleCmd.substr(4, 2));
            // serial.writeString(index.toString());
            // serial.writeString(NEW_LINE);
            let cmd: string = handleCmd.substr(0, index * 2 + index - 1);
            // serial.writeString(cmd);
            // serial.writeString(NEW_LINE);
            for (let i = 0; i < index * 2 + index - 3; i += 2) {
                // serial.writeString(strToNumber(cmd.substr(i, 2)).toString());
                // serial.writeString(NEW_LINE);
                j = j + strToNumber(cmd.substr(i, 2));
            }
            if (strToNumber(cmd.substr(index * 2 + index - 3, 2)) == (j & 0xFF) && strToNumber(cmd.substr(index * 2 + index - 3, 2)) != 0) {
                serial.writeString("Ture");
            }
            else if (strToNumber(cmd.substr(index * 2 + index - 3, 2)) != (j & 0xFF) && strToNumber(cmd.substr(index * 2 + index - 3, 2)) != 0) {
